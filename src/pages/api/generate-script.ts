...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    let script = Array.isArray(data) && data[0]?.generated_text
      ...
      : `In the world of sports, few names inspire greatness like ${celebrity}. Rising from humble beginnings, ${celebrity} captured the hearts of fans with unforgettable performances and relentless determination. From record-breaking achievements to iconic championship moments, ${celebrity} has redefined what it means to be a legend. Through every victory and challenge, ${celebrity} continues to inspire the next generation. ${celebrity} — a true icon whose legacy will live on forever.`;

    if (!script || script.length < 30) {
      script = `In the world of sports, few names inspire greatness like ${celebrity}. Rising from humble beginnings, ${celebrity} captured the hearts of fans with unforgettable performances and relentless determination. From record-breaking achievements to iconic championship moments, ${celebrity} has redefined what it means to be a legend. Through every victory and challenge, ${celebrity} continues to inspire the next generation. ${celebrity} — a true icon whose legacy will live on forever.`;
    }
