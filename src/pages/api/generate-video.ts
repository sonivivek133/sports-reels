...
async function downloadImagesWithFallbacks(celebrity: string): Promise<Uint8Array[]> {
  ...
  if (NBA_PLAYER_IDS[celebrity]) {
    try {
      const nbaUrl = `https://cdn.nba.com/headshots/nba/latest/1040x760/${NBA_PLAYER_IDS[celebrity]}.png`;
      const nbaImg = await axios.get(nbaUrl, { responseType: 'arraybuffer' });
      images.push(new Uint8Array(nbaImg.data));
    } catch {}
  }
  ...
  try {
    ...
    for (const page of Object.values(pages)) {
      ...
      if (url && /\.(jpg|jpeg|png)$/i.test(url)) {
        try {
          const imgResp = await axios.get(url, { responseType: 'arraybuffer' });
          images.push(new Uint8Array(imgResp.data));
...
async function createSlideshowWithAudio(
  ...
  // Write images and audio to ffmpeg FS
  for (let i = 0; i < images.length; i++) {
    ffmpeg.FS('writeFile', `img${i}.jpg`, images[i]);
  }
  ...
  let concatList = '';
  for (let i = 0; i < images.length; i++) {
    concatList += `file img${i}.jpg\nduration ${IMAGE_DURATION}\n`;
  }
  // Repeat last image for duration
  concatList += `file img${images.length - 1}.jpg\n`;

  ffmpeg.FS('writeFile', 'input.txt', concatList);

  // Generate slideshow video (no audio yet)
  await ffmpeg.run(
    '-f', 'concat',
    '-safe', '0',
    '-i', 'input.txt',
  ...
  // Cleanup FS
  ffmpeg.FS('unlink', 'slideshow.mp4');
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (typeof audioBuffer === 'string' && audioBuffer.startsWith('data:')) {
      const base64Data = audioBuffer.split('base64,')[1];
      if (!base64Data) return res.status(400).json({ error: 'Invalid Base64 audioBuffer format' });
      audioData = new Uint8Array(Buffer.from(base64Data, 'base64'));
    } else if (Array.isArray(audioBuffer)) {
      try {
    ...
    for (let reelIdx = 0; reelIdx < REEL_COUNT; reelIdx++) {
      ...
      const outputName = `final-${sanitizedName}-reel${reelIdx}-${timestamp}.mp4`;

      const videoData = await createSlideshowWithAudio(images, audioData, outputName);
